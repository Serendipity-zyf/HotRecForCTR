...
logger = ColorLogger(name="TrainScriptAmazon")


@Registers.train_script_registry.register
class SingleGPUTrainScriptAmazon(object):
    """Single GPU / CPU training script for Amazon Dataset."""
    ...
    def __init__(
        ...
        self.name = "SingleGPUTrainScriptAmazon"
        self.epochs = epochs
    ...
    def _train_epoch(
        ...
        with ProgressBar(total=len(train_loader), title=f"Training") as bar:
            for batch in train_loader:
                ...
                # Forward pass
                optimizer.zero_grad()
                output = model(uid_idx, iid_idx, cate_idx, seq_idx, seq_cat_idx, mask, dense)
    ...
    @classmethod
    def from_config(cls, config: Dict[str, Any]) -> "SingleGPUTrainScriptAmazon":

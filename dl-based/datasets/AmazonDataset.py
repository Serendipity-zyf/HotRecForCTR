...
class AmazonDataset(Dataset):
    ...
    def __init__(
        ...
        if uid_vocab is not None and item_vocab is not None:
            ...
            self.uid_vocab, self.item_vocab = self._load_vocab()
    ...
    def _load_vocab(self) -> Tuple[UIDVocab, UIDVocab]:
        ...
        return (UIDVocab.load(self.user_vocab_path), UIDVocab.load(self.item_vocab_path))
    ...
    def collate_fn(batch: List[Tuple[torch.Tensor, ...]]) -> Tuple[torch.Tensor, ...]:
        ...
        PADING_VALUE = 0
        uid_idx, iid_idx, seq_idx, valid_len = zip(*[item[0] for item in batch])
        ...
        return (uid_idx, iid_idx, seq_idx, mask), dense, label

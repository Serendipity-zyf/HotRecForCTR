...
class ModelAnalyzer:
    ...
    def _get_child_modules(self, name: str) -> List[str]:
        ...
        if name == "":
            ...
            try:
                ...
                children = [f"{name}.{child_name}" for child_name in module._modules.keys()]

                # Filter out children that don't exist in module_info
                return [child for child in children if child in self.module_info]
            except (AttributeError, KeyError):
                # Fall back to the old method if we can't find the module
    ...
    def _format_module_tree(self, name: str = "", depth: int = 0, max_depth: int = 3) -> List[str]:
        ...
        if name == "":
            ...
            for param_name, param_info in direct_params:
                ...
                self.module_info[f"_param_{param_name}"] = {
                    ...
                    "trainable_params": (param_info["param"].numel() if param_info["trainable"] else 0),
        ...
        return self._format_module_tree_recursive(name, depth, max_depth, False, prefix_dict, col_widths)
    ...
    def _format_module_tree_recursive(
        ...
        # Check if the module info exists
        if name not in self.module_info:
            # Skip this module if it's not in module_info
            return []
    ...
    def summary(self, max_depth: int = 3) -> str:
        ...
        forward_backward_size = self.activation_memory_size * 2  # Account for both activations and gradients
